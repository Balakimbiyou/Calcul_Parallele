...
class Grille:
    ...
    def __init__(self, rank: int, nbp: int, dim, init_pattern=None, color_life=pg.Color("black"),
        ...
        print(
            f"rank {rank} : dimensions locales : {self.dimensions_loc}, start_loc_row : {self.start_loc_row}, star_loc_col : {self.start_loc_col}")
...
class App:
    ...
    def draw(self):
        surface = pg.surfarray.make_surface(self.colors[self.grid.cells[1:-1, 1:-1].T])
    ...
    if rank == 0:
        ...
        while loop:
            ...
            if full_grid is not None:
                appli.grid.cells[1:-1, 1:-1] = full_grid
        ...
        while loop:
            ...
            # Rassembler toutes les colonnes locales (sans ghost cells) sur le rang 0 de newCom
            local_block = np.ascontiguousarray(grid.cells[1:-1, 1:-1])
            all_blocks = newCom.gather(local_block, root=0)
            all_starts = newCom.gather((grid.start_loc_row, grid.start_loc_col), root=0)

            if newCom.rank == 0:
                full_grid = np.zeros((grid.dimensions[0], grid.dimensions[1]), dtype=np.uint8)
                for block, (s_row, s_col) in zip(all_blocks, all_starts):
                    h, w = block.shape
                    full_grid[s_row:s_row + h, s_col:s_col + w] = block
